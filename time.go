...
func main() {
  ...
  // day, etc. Times are always associated with a Location, ie,
  // time zone.
  then := time.Date(
  ...
  // The Sub method returns a Duration representing the
  // interval between two times
  diff := now.Sub(then)
  p(diff)

  // We can computer the length of the duration in various
  // units.
  p(diff.Hours())
  p(diff.Minutes())
  p(diff.Seconds())
